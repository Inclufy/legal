# Briefing & Non-negotiables — voor Sami, persoonlijk

> Dit document is voor jouw ogen alleen. Niet delen. Niet aan tafel laten liggen.
> Lees het vannacht door, en print het of neem het mee als referentie tijdens overmorgen.

---

## Multi-region architectuur

Inclufy zet **meerdere regionale entiteiten** op. Deze kickoff betreft uitsluitend **Inclufy Marokko/Afrika**:

| Entiteit | Aandeelhouders | Status |
|---|---|---|
| Inclufy BV (NL) | Inclufy alleen — NL-markt zonder externe partners | Bestaat |
| **Inclufy Marokko/Afrika** | Inclufy + Said (+ Bennani?) + investeerder | **Start nu — model TBD** |
| Inclufy Midden-Oosten | Inclufy + lokale partners TBD + investeerder | Verkenning |
| Inclufy USA | Inclufy + lokale USA-partner + investeerder | Verkenning |

## Twee modellen voor Marokko/Afrika

Er liggen **twee structurele opties** op tafel — beslissing pas na één-op-één met Bennani:

- **Model A — met Bennani als JV-partner**: Inclufy + Said + Bennani + investeerder (3 founders + investeerder).
- **Model B — zonder Bennani**: Inclufy + Said + investeerder (2 founders + investeerder).
- **Model C — hybride**: JV als Model B, Bennani als reseller met commissie zonder aandelen.

**Zie [09-model-vergelijking.md](./09-model-vergelijking.md) voor volledige analyse en beslissingsmatrix.**

Investeerder participeert sowieso. Said sowieso. Bennani's vorm is de variabele.

---

## Wat we nu definitief weten

**Morgen** — technische kickoff met sales rep van Dar Group. Alleen operationeel.
On-prem deployment op lokale server. Geen commerciële discussies.

**Overmorgen** — één-op-één met Said, eigenaar Dar Group. Strategisch gesprek.
Geen JV-contract, geen aandelen-percentages, geen exclusiviteit. Wel: principes en proces.

**Said's profiel — wat je moet weten voor de toon:**
- Nederlandse ondernemer, gevestigd in **Heerhugowaard (NL)**.
- **Bedrijfsstructuur**: **Dasa Group** in Nederland; **Dar Group** als de Marokkaanse entiteit/operatie.
- **Eigenaar van een industrieel printbedrijf** met 10+ jaar operationele aanwezigheid in Marokko.
- Voert **secure print** uit: stembiljetten voor meerdere Afrikaanse landen, productlabels voor supermarkten, andere beveiligde print-diensten.
- Heeft **directe relaties met Marokkaanse en Afrikaanse overheden** en grote bedrijven.
- Werkt al substantieel samen met **Bennani** — hun samenwerking is geen nieuw construct, het is bestaande chemie.
- Dit is een **peer-ondernemer**, geen prospect. Behandel hem als gelijke. Hij verwacht efficiëntie en directe taal, geen pitch-toon.
- Government-track-record + cross-African footprint zijn **strategische assets** voor de JV — dat moet in de waardering meegenomen worden.
- **Voordeel**: Said is een NL-gevestigde ondernemer — cross-border structurering tussen Inclufy BV en de Marokkaanse JV is voor hem vertrouwd terrein. NL ↔ MA fiscaal verdrag, transfer pricing, holding-structuren — hij kent het.

**Bennani** — geïntroduceerd via Said, gesproken met Said erbij (twee keer).
Toonde interesse om klanten in te brengen. Bracht zelf het onderwerp "geld inleggen voor sales-capaciteit" op.
**Nog niet één-op-één gesproken — dit is een gat dat binnen 10 dagen gedicht moet.**

**Bennani's positie:**
- Werkt voor **TechDesk Casablanca** (rol nog te verifiëren — werknemer of mede-eigenaar?).
- **Mede-eigenaar van een Ricoh multicopier-distributeur** in Marokko.
- Heeft dus B2B channel-sales-ervaring en een bestaande klantbase van bedrijven (kantoren, organisaties met printers — overlappende prospects voor finance/ERP).
- **Open vraag**: wat is zijn primaire business — TechDesk, de Ricoh-distributorship, of Inclufy MENA? Hoeveel tijd kan hij realistisch toewijzen aan de JV?
- **Open vraag**: is de Ricoh-distributorship cashflow-positief — zo ja, betekent dat hij de €33k cash-inleg kan dragen.

**Investeerder** — eigen netwerk, interesse maar geen ondertekend stuk.
Range: €100k–€500k over 3 jaar. **Multi-region scope**: Marokko/Afrika nu, Midden-Oosten en USA later. Voorstel moet nog worden uitgewerkt.

**Hasna (Dar Group)** — mogelijk verkoopkanaal voor Dar Group's bestaande relaties.
Rol nog niet geformaliseerd. Kan sales-rep onder Dar Group worden, commissie-channel, of klein deel van toekomst-pool. **Niet vandaag uitwerken — wel benoemen.**

---

## De afgesproken structuur (zoals je het nu begrijpt)

| Functie | Wie | Hoe |
|---|---|---|
| Kennis, training, implementatie | Inclufy | Eigen werk + IP-licentie |
| Sales | Said + Bennani | Eigen mensen + klantintroducties |
| Lokaal support team | Aan te nemen | €100k drie-partijen-inleg |
| JV management | Said + Bennani | Operationele leiding |
| Groei / opschaling | Investeerder | €100k–€500k getranchet |

**De €100k cash-inleg is het fundament.** Drie partijen, elk ongeveer €33k.
Bennani heeft dit zelf opgebracht. Said heeft eraan meegewerkt. Jij hebt ingestemd.

---

## Wat je NIET weet (en overmorgen niet impliciet als bekend mag aannemen)

- Hoeveel klanten Said + Bennani realistisch in jaar 1 binnenbrengen
- Of het lokale support team bedoeld is voor delivery (Inclufy-rol) of alleen support (lokale rol)
- Wie operationeel CEO van de JV wordt (Said? Bennani? Co-leadership?)
- Bennani's onafhankelijke beeld (hij heeft alleen met Said erbij gesproken)
- Of Said cash beschikbaar heeft voor zijn €33k of het uit JV-omzet wil betalen
- Hoe Inclufy structureel gecompenseerd wordt voor delivery-uren in jaar 2/3
- **Hoeveel tijd Said realistisch toewijst** — hij heeft een draaiend industrieel printbedrijf met cross-Afrikaanse operaties
- **Of zijn government-relaties in Afrikaanse landen actief inzetbaar zijn voor Inclufy MENA** — dit is potentieel een enorm strategisch asset, of alleen relevant voor zijn print-business
- **Welke entiteit aandeelhouder wordt** — Dasa Group (NL), Dar Group (MA), of Said persoonlijk. Heeft fiscale en governance-implicaties.

**Stel deze vragen overmorgen. Schrijf de antwoorden op.**

---

## Non-negotiables — jouw kompas

### 1. IP blijft 100% bij Inclufy BV

Niet onderhandelbaar. De JV krijgt een licentie voor Marokko (eventueel later uitgebreid).
Geen IP-overdracht in welke vorm dan ook. Geen co-ownership van software, methodologie of brand.

> Als iemand probeert dit te bespreken: *"Dat is voor ons en voor de financierbaarheid van de JV een vast uitgangspunt. Andere internationale software-partnerships zijn op dezelfde manier opgezet."*

### 2. Geen aandelen-percentages overmorgen

Geen ranges, geen "ongeveer", geen indicatieve cijfers. Alles wat ik nu in mijn hoofd heb (Inclufy 40–45%, Said 22–27%, Bennani 15–20%, investeerder 12–18%, pool 5%) is **alleen kompas**, niet voor aan tafel.

> Als iemand erop aandringt: *"Cijfers volgen uit de waardering, en de waardering doen we met juridische en fiscale begeleiding. Vandaag focussen we op principes."*

### 3. Geen exclusiviteit zonder commitments

Wederzijdse exclusiviteit voor Marokko is bespreekbaar — gekoppeld aan minimum sales-targets en heldere herzieningsmomenten. Geen exclusiviteit langer dan 12 maanden zonder verlenging op basis van prestatie. Geen MENA-brede exclusiviteit voor één partij.

### 4. Geen handtekening overmorgen

Geen term sheet, geen MOU, geen LOI, geen "principe-akkoord op papier". Mondelinge bevestiging van richting is voldoende. Schriftelijke samenvatting volgt na het gesprek (van mij), niet ervoor.

### 5. Drie partijen leggen alle drie cash in

Niet alleen tijd. Niet alleen "value". De €100k drie-partijen-inleg is cash op de bank van de JV bij oprichting. Als Said of Bennani op dit punt draaien — dat is een rode vlag die je serieus moet nemen.

### 6. Investeerder financiert de JV, niet Inclufy BV

Glashelder, vanaf moment één. De investeerder koopt aandelen in de Marokkaanse JV (of in een MENA-holding eronder), niet in Inclufy BV. Misverstand hierover later corrigeren is buitengewoon moeilijk.

### 7. Ik ontmoet Bennani één-op-één voordat we juridisch structureren

Niet onderhandelbaar. Geen JV met iemand die ik niet onafhankelijk ken.

### 8. Inclufy wordt structureel betaald voor delivery-uren

Implementation fee per klant óf intercompany rate per uur. Niet "onderdeel van de licentie" — anders ben ik in jaar 2 de gratis bottleneck.

### 9. License Fee Deferral — bestaansrecht-mechanisme

Eerste 2 jaar of 15 betalende klanten (whichever first): JV betaalt €0 license fees aan Inclufy. Implementation fees blijven betaalbaar. Versterkt JV-runway met 50–80%, signaleert founder commitment. Drie randvoorwaarden niet-onderhandelbaar:
- Implementation fees blijven door (delivery-werk wordt betaald)
- "Whichever first" trigger (niet "whichever last")
- Klant-definitie strikt: paying, distinct, active, ≥€500/maand ARR, exclusief Dar Group pilot

Zie [09 model-vergelijking](./09-model-vergelijking.md) en [10 license-fee-deferral](./10-license-fee-deferral.md) voor detail.

---

## Rode vlaggen om in de gaten te houden — overmorgen

- **Said draait op de €33k cash-inleg.** "We doen het in tijd in plaats van cash" → niet OK. Vraag waarom, wat heeft veranderd.
- **Said weigert dat ik Bennani één-op-één spreek.** "Dat hoeft niet" of "dat regelen we later" → ongezonde controle over communicatie.
- **Drukken op snelle handtekening.** "Kunnen we het vandaag al vastleggen?" → vertraag, professioneel.
- **Vragen over investeerder die op Inclufy BV-niveau lijken.** "Hoeveel aandelen in Inclufy krijgt hij?" → corrigeer direct: investeerder zit in JV.
- **Bennani's bijdrage wordt opeens groter dan eerder besproken.** Said schuift waarde rond → vraag door.
- **Tijdsdruk op de pilot.** "We willen al binnen 4 weken live" → onrealistisch, beschadigt de partnership.
- **Bennani is full-time in zijn Ricoh-business.** Als hij geen substantiële tijd vrij kan maken, is zijn aandeel als operationeel mede-leider niet te rechtvaardigen → herzie zijn rol naar "channel partner met klantintroductie" in plaats van "mede-CEO".

---

## Exacte zinnen voor moeilijke momenten

**Als ze cijfers vragen:**
> "Cijfers werk ik uit in het commercial framework de komende weken. Vandaag wil ik vooral principes en richting bevestigen."

**Als ze handtekening vragen:**
> "Ik wil dit goed doen, niet snel. Een gehaast getekende structuur is voor niemand goed. Geef me de tijd om het netjes uit te werken — binnen 8–12 weken ligt er een serieus stuk."

**Als ze exclusiviteit vragen:**
> "Exclusiviteit voor Marokko is wat mij betreft bespreekbaar in het partnership-framework. Met heldere targets en herzieningsmomenten — dat beschermt jullie ook, want het zorgt dat de exclusiviteit gegrond is op werkelijke samenwerking."

**Als ze investeerder-details vragen:**
> "Het exacte ticket en de voorwaarden werk ik uit in het voorstel. Wat ik nu kan zeggen: het is voldoende voor een gezonde opzet en groei in lijn met het businessplan dat we samen ontwikkelen."

**Als Said het over Bennani heeft alsof hij voor hem spreekt:**
> "Goed dat je dit deelt. Voordat we Bennani's positie formaliseren wil ik graag persoonlijk met hem zitten — net zoals jij ongetwijfeld op enig moment met mijn investeerder zou willen spreken."

**Als de investeerder ter sprake komt en ze vragen "is hij erbij?":**
> "Hij is geïnteresseerd en we werken aan het voorstel. Ik betrek hem op het juiste moment in het traject."

**Als ze pushen om méér dan principes vast te leggen:**
> "Ik begrijp de wens om snel concreet te worden. Mijn voorstel: vandaag bevestigen we richting en principes, en ik kom binnen 2 weken met een uitgewerkt commercial framework dat we samen aanscherpen."

---

## Eindcheck voor jezelf — voordat je overmorgen de kamer in loopt

- [ ] Heb ik de zes principes uit mijn hoofd geleerd?
- [ ] Weet ik welke vragen ik aan Said wil stellen?
- [ ] Heb ik Bennani's contactgegevens, of moet ik die nog opvragen?
- [ ] Is mijn investeerder geïnformeerd over de regionale richting?
- [ ] Heb ik een leeg notitieboek of laptop voor het opschrijven van Said's antwoorden?
- [ ] Heb ik een schriftelijke samenvatting-template klaar voor na het gesprek?
- [ ] Ben ik comfortabel met "we leggen niets vast vandaag"?

---

## Eén persoonlijke herinnering

> Goede deals worden langzaam gemaakt.
> Vertraag elke keer dat je de neiging voelt om iets weg te geven om de relatie warm te houden.
> Said is een ervaren ondernemer. Hij verwacht structuur, geen concessies.
