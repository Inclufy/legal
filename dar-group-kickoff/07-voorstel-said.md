# Partnership Voorstel — Dasa Group (NL) / Dar Group (Marokko) — Said

> **Status:** Werkdocument / draft. Bedoeld als basis voor het strategisch gesprek met Said.
> Niet ondertekend, niet bindend. Concrete cijfers zijn **indicatief** en worden in finalisatie aangepast.

---

## 1. Visie

Inclufy en Dasa/Dar Group bouwen samen — met TechDesk/Bennani als mede-partner en een externe investeerder voor groei — **Inclufy Marokko/Afrika**: een nieuwe Marokkaanse entiteit die de Marokkaanse en Afrikaanse markt ontsluit voor moderne, lokaal-compliant finance/ERP-software.

**Bredere context:** dit is de **eerste van meerdere regionale entiteiten** die Inclufy opzet — naast Marokko/Afrika ook Midden-Oosten (lokale partners TBD) en USA (met lokale USA-partner). De NL-markt blijft door Inclufy BV zelfstandig bediend zonder externe partners. Said en Bennani zijn aandeelhouders in **uitsluitend de Marokko/Afrika-entiteit**. Dasa/Dar Group is de eerste regionale anker-partner van Inclufy's mondiale strategie.

**Said's positie:** strategische anker-partner voor Marokko/Afrika en mede-architect van een regionaal succes. Zijn 10+ jaar operationele aanwezigheid in Marokko (industrieel printbedrijf, secure print, stembiljetten in meerdere Afrikaanse landen, supermarkt-labels) en zijn government/enterprise-netwerk vormen substantieel kapitaal dat de JV vanaf dag één toegang geeft tot besluitvormers die nieuwe spelers maanden tot jaren zou kosten om te bereiken. Sub-Sahara Afrika via Marokko als hub is een natuurlijke uitbreiding.

---

## 2. Wederzijdse uitgangspunten

| Inclufy brengt | Dasa/Dar Group brengt |
|---|---|
| Bewezen software-product | 10+ jaar operationele track-record in Marokko |
| IP en mondiale productroadmap | Government- en enterprise-netwerk (MA + cross-Afrika) |
| Kennis, training, implementatie | Sales-kanaal en marktreputatie |
| Investeerder uit eigen netwerk | Operationele JV-leiding |
| Cash-inleg van ~€33k voor sales-team jaar 1 | Cash-inleg van ~€33k voor sales-team jaar 1 |
| Regionale ambitie (MENA + Afrika) | Lokale aanwezigheid en cultuurkennis |
| NL-entiteit voor cross-border structurering | NL-entiteit (Dasa Group) + MA-operatie (Dar Group) — vertrouwd terrein voor cross-border holding |

---

## 3. De drie sporen

### Spoor 1 — Pilot Implementatie (start nu)
- **Wat:** Dar Group als referentieklant van Inclufy. Implementatie van Finance/ERP core.
- **Scope:** GL, AP, AR, banking, TVA, états de synthèse — Phase 1.
- **Tijdlijn:** Go-live binnen 90 dagen.
- **Condities:** Founding Partner pricing — implementatie voor onze rekening, licenties met substantiële korting (te finaliseren in commercial framework).
- **Voor jullie:** referentie-implementatie + interne digitalisering + leerervaring voor sales naar derden.
- **Voor ons:** lighthouse-case voor de markt + product-feedback voor lokalisatie.

### Spoor 2 — Commercial Framework (4–6 weken)
- **Wat:** Operationeel kader voor de samenwerking — transfer pricing, marges, support-modellen, territoriale rechten.
- **Doel:** vóór JV-formalisatie helderheid hebben over hoe geld stroomt, wie wat doet, wat exclusief is.
- **Werkwijze:** gezamenlijke werksessies, geen juridische zwaarte, voorbereiding op spoor 3.

### Spoor 3 — JV & Investering (8–12 weken)
- **Wat:** Juridische structurering met Marokkaanse en Nederlandse advocaten/fiscalisten.
- **Resultaat:** aandeelhoudersovereenkomst, statuten, IP-licentie, investeringsovereenkomst.
- **Mijlpaal:** closing en eerste tranche van investeerder.

---

## 4. JV-structuur — uitgangspunten

### 4.1 Naam (werknaam)
**Inclufy MENA** — definitieve naam af te stemmen.

### 4.2 Rechtsvorm
Marokkaanse SARL of SA, te bepalen op fiscale en governance-overwegingen samen met juristen.

### 4.3 IP-licentie
- Inclufy BV verleent exclusieve licentie aan de JV voor commercialisatie in **Marokko en gedefinieerde Afrikaanse landen** (initiële lijst af te stemmen, verlengbaar bij prestatiemilestones).
- **Geen exclusiviteit voor Midden-Oosten of USA** — die markten worden door aparte regionale entiteiten bediend met andere lokale partners.
- Looptijd: **5 jaar**, automatisch verlengbaar.
- Royalty-vrij voor de JV — de transfer price op licentie-inkoop dekt Inclufy's vergoeding.

### 4.4 Aandelenstructuur (indicatief)

| Aandeelhouder | % (range) | Bijdrage |
|---|---|---|
| Inclufy | 40–45% | IP-licentie + cash + delivery |
| Dasa/Dar Group | 22–27% | Cash + sales + delivery + consulting + JV management + government/enterprise-netwerk |
| TechDesk / Bennani | 15–20% | Cash + sales + klantintroductie + mede-management |
| Investeerder | 12–18% | Cash voor groei (€100k–€500k getranchet) |
| Pool toekomst | 5% | Reserve voor key hires |

> **Definitieve percentages volgen uit formele waardering** met juridische en fiscale begeleiding. Dasa/Dar Group's percentage is hoger dan een puur sales-partner zou zijn, vanwege (a) Said's track-record en credibility in de Marokkaanse markt en (b) de strategische waarde van zijn government/enterprise-relaties voor klant-acquisitie.

### 4.5 Vesting
- Aandelen voor Dasa/Dar Group **vesten over 4 jaar** (cliff van 1 jaar) gekoppeld aan operationele bijdrage:
  - Sales-doelen (klanten binnenbrengen)
  - Delivery-uren (consulting capaciteit toewijzen)
  - JV-management commitment (continuïteit van leiding)
- Bij niet-leveren: clawback van niet-vested aandelen.

---

## 5. Financiële opzet

### 5.1 Drie-partijen seed (€100k cash)
- Inclufy: ~€33k
- Dar Group: ~€33k
- TechDesk: ~€33k
- **Doel:** lokaal support team in jaar 1 (2 sales-/support-mensen, gedeelde verantwoordelijkheid).
- **Storting:** bij oprichting JV.

### 5.2 Investeerders-tranches (€100k–€500k over 3 jaar)
- **Tranche 1**: bij oprichting + handtekening shareholder agreement.
- **Tranche 2**: bij eerste 5 betalende klanten.
- **Tranche 3**: bij verdere groeimijlpaal of voorbereiding 2e markt.

### 5.3 Inkomsten- en margestructuur (indicatief)
- **Software subscription**: JV koopt licenties bij Inclufy in tegen ~45% van listprijs; verkoopt met ~55% bruto marge.
- **Implementation fees**: door JV uitgevoerd; Inclufy ontvangt **implementation fee per klant** voor kennis-, training- en implementatie-uren (te finaliseren in commercial framework).
- **Consulting**: 100% JV-omzet.
- **Academy**: gedeelde inkomstenstroom — content-eigendom Inclufy, lokale levering door JV met afgesproken split.

---

## 6. Rolverdeling — operationeel

| Functie | Verantwoordelijke | Toelichting |
|---|---|---|
| Productontwikkeling, IP, brand | Inclufy | Geen JV-bemoeienis |
| Sales en accountmanagement | Dar Group + TechDesk + (Hasna voor Dar Group-relaties) | Eigen mensen, eigen kanalen |
| JV-leiding (CEO of co-leadership) | Dar Group + TechDesk | Operationeel verantwoordelijk |
| Implementatie en training | Inclufy | Tegen implementation fee per klant |
| 1e-lijn lokaal support | Lokaal support team | Gefinancierd uit drie-partijen seed |
| 2e/3e-lijn support | Inclufy | Tegen retainer of incident-basis |
| Marketing en lokale communicatie | JV met Inclufy-input | Brand guidelines van Inclufy |
| Academy levering | JV | Content geleverd door Inclufy |

> **Hasna's rol** — als Hasna Inclufy aan Dar Group's relaties gaat verkopen, formaliseren we dit in het commercial framework: sales-rep onder Dar Group met JV-commissie, of commissie-gebaseerde channel-arrangement. Niet vandaag uitwerken — wel benoemen.

---

## 7. Governance

### 7.1 Board
- 4–5 leden: 2 Inclufy, 1 Dar Group, 1 TechDesk, optioneel 1 investeerder.
- Voorzitter wisselend of door Inclufy benoemd, te bespreken.
- Quarterly meetings (in persoon of video).

### 7.2 Reserved matters
**Vereisen meerderheid + Inclufy-instemming:**
- Wijzigingen aan IP-licentie of productroadmap
- Brand- en communicatie-policy
- Strategische uitbreiding naar nieuwe markten
- Wijzigingen aan aandeelhoudersovereenkomst

**Vereisen meerderheid + investeerder-instemming:**
- Budget >€X
- Key hires (CEO, COO, CFO)
- Aanvullende financiering
- Vervreemding van substantiële activa
- Exit-besluiten

### 7.3 Operationele beslissingen
Dagelijkse beslissingen door JV-leiding zonder board-goedkeuring tot een vooraf afgesproken plafond.

---

## 8. Exclusiviteit en territorium

- **Territorium van Marokko/Afrika-entiteit**: Marokko en gedefinieerde Afrikaanse landen (initiële lijst af te stemmen).
- **Wederzijdse exclusiviteit binnen dit territorium** gedurende eerste 12 maanden vanaf JV-oprichting; verlengbaar bij behalen minimum sales-targets.
- **Geen exclusiviteit voor Midden-Oosten** — die markt wordt door een aparte regionale entiteit van Inclufy bediend met eigen lokale partners.
- **Geen exclusiviteit voor USA** — die markt wordt door een aparte regionale entiteit met eigen lokale USA-partner bediend.
- **Geen exclusiviteit voor NL** — Inclufy BV bedient de NL-markt zelfstandig zonder externe partners.

---

## 9. Investeerder

- Investeerder uit Inclufy's netwerk.
- Ticket: **€100k–€500k** getrancheerd over 3 jaar, gekoppeld aan milestones.
- Investering zit **in de JV**, niet in Inclufy BV.
- Investeerder krijgt aandelen, board-rol (optioneel), reserved matters, exit-rechten.
- Inclufy beheert de relatie met de investeerder; Dar Group en TechDesk worden op afgesproken momenten betrokken.

---

## 10. Tijdlijn — komende 12 weken

| Week | Mijlpaal |
|---|---|
| 1–2 | Pilot kickoff, on-prem deployment Dar Group |
| 1–2 | Eén-op-één gesprek Inclufy ↔ Bennani |
| 3–4 | Commercial framework eerste werkversie |
| 5–6 | Marokkaanse advocaat aangesteld; gezamenlijke kick-off JV-structurering |
| 7–8 | Commercial framework gefinaliseerd |
| 8–10 | Term sheet JV-overeenkomst |
| 9–11 | Due diligence wederzijds |
| 11–12 | Definitieve documentatie en handtekening |
| Pilot parallel | Go-live Dar Group rond week 12 |

---

## 11. Wat we nu vastleggen — vandaag

> **Dit document is geen contract.** Vandaag bevestigen we dat dit het kader is waarbinnen we de komende weken werken. Geen handtekening, geen percentages vastgepind, geen exclusieve commitments.

**Bevestigen vandaag:**
- [ ] De drie sporen en hun tempo
- [ ] De zes samenwerkings-principes
- [ ] De drie-partijen-cash-inleg van €100k
- [ ] IP blijft bij Inclufy BV
- [ ] Investeerder zit in de JV, niet in Inclufy BV
- [ ] Bennani-gesprek één-op-één in komende 2 weken
- [ ] Eigen advocaten aan beide kanten
- [ ] Welke entiteit aandeelhouder wordt — Dasa Group (NL), Dar Group (MA), of Said persoonlijk

**Open punten voor commercial framework:**
- Exacte transfer pricing en marges
- Implementation fee per klant
- Support SLA en pricing
- Exclusiviteit-targets en herzieningsmomenten
- Definitieve aandelenverhoudingen

**Open punten voor JV-structurering:**
- Rechtsvorm (SARL of SA)
- Specifieke governance-clausules
- Vesting-schema en clawback-mechanisme
- Exit en liquidity-mechanismen
- Belastingoptimalisatie cross-border

---

## Volgende stappen

1. Schriftelijke samenvatting van het gesprek (binnen 24 uur, van Sami naar Said).
2. Eén-op-één Inclufy ↔ Bennani (komende 2 weken).
3. Eerste werkversie commercial framework (van Sami, binnen 3 weken).
4. Aanstellen Marokkaanse advocaat (binnen 4 weken).
5. Gezamenlijke kick-off JV-structurering (binnen 6 weken).

---

> **Disclaimer:** Dit document dient ter bespreking. Geen contractuele binding. Definitieve afspraken volgen uit ondertekende JV-overeenkomst en bijbehorende documentatie.
